...
SEED_SQL="$ROOT/demo/infra/iceberg/sql/seed_aml_from_hive_files.sql"
...
python3 "$ROOT/demo/infra/scripts/generate_iceberg_seed_csv_connector.py" \
  --csv "$CSV_PATH" \
  --rows "$MAX_ROWS" \
...
if [[ "$FILE_FORMAT" == "json" ]]; then
  ...
  python3 "$ROOT/demo/infra/scripts/convert_seed_csv_to_jsonl.py" --in-dir "$RAW_CSV_DIR" --out-dir "$RAW_JSON_DIR"
fi
end_gen=$(date +%s.%N)
...
python3 "$ROOT/demo/infra/scripts/generate_iceberg_seed_hive_sql.py" \
  --out "$SEED_SQL" \
  --format "$FILE_FORMAT" \
  --raw-base "$RAW_BASE"
end_sql=$(date +%s.%N)
